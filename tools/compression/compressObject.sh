...
run_()
{
  ...
  #echo "${tabulation}$CompressionMode"
	...
	# (7fe0,0010) Pixel Data
 	ImageInfo=`$DCMTK_HOME/bin/dcmdump +P "0002,0010" +P "7fe0,0010" $Input 2>/dev/null`

	#[ -n "$Verbose" ] && echo "$ImageInfo"

	if ! grep -q "7fe0,0010" <<<"$ImageInfo"; then
		#[ -n "$Verbose" ] && echo "${tabulation}no image..."
    echo "${tabulation}$Input - no image..."
		return 0
	fi
  ...
  currentCompression=$(echo "$ImageInfo" | grep "0002,0010" | awk '{print $1, $2, $3}')
	...
	if ! grep -q "0002,0010.*Endian" <<<"$ImageInfo"; then
		#[ -n "$Verbose" ] && echo "${tabulation}Already Compressed/Encapsulated..."
    echo "${tabulation}$Input - Already Compressed/Encapsulated...$currentCompression"
		return 0
	fi
  ...
  # COMPERSS VIA JPEGLOSSLESS
    ...
    #echo "${tabulation}command: dcmcjpeg ${CompFlag} $Input ${CompressedTempFile}"
		...
		if [ $? -ne 0 ] ; then
			...
			#[ -n "$Verbose" ] && echo "${tabulation}compression failed"
      echo "${tabulation}$Input - compression failed"
			return 1
		fi
	fi
  ...
  #echo "${tabulation}command: objdiff ${Input} ${CompressedTempFile}"
	...
	if [ $? -ne 0 ] ; then
	  [ -f "${CompressedTempFile}" ] && rm "${CompressedTempFile}"
	  #[ -n "$Verbose" ] && echo "${tabulation}objdiff failed (${CompressedTempFile})"
      echo "${tabulation}$Input - objdiff failed (${CompressedTempFile})"
	  return 1
	fi
  ...
  if [[ $compSize -lt $origSize ]]; then
    ...
    if [[ "$Verbose" == "true" ]]; then
      ...
      echo "${tabulation}$Input - Compression success. FROM $beforeSize $beforeUnit TO $afterSize ${afterUnit}, RATIO ${compRatio}, SAVED SPACE ${spaceSavings}%."
    else
      echo "${tabulation}$Input - Compression success. FROM $beforeSize $beforeUnit TO $afterSize ${afterUnit}, RATIO ${compRatio}, SAVED SPACE ${spaceSavings}%."
    fi
    ...
    echo "${tabulation}$Input - compressed size($afterSize $afterUnit) is greater than original size ($beforeSize $beforeUnit)"
    [ -f "${CompressedFile}" ] && rm "${CompressedFile}"
    return 1
  fi
	...
	if [ -n "${CompressedFile}" ] ; then 
		...
		if [ $? != 0 ] ; then
			#[ -n "$Verbose" ] && echo "${tabulation}Copying compressed object to target directory failed."
      echo "${tabulation}$Input - Copying compressed object to target directory failed."
			[ -f "${CompressedFile}" ] && rm "${CompressedFile}"
		...
		if [ $? == 0 ] ; then
			...
			#[ -n "$Verbose" ] && echo "${tabulation}Renaming temp file to $Output failed."
      echo "${tabulation}$Input - Renaming temp file to $Output failed."
			return 1
		fi
	fi
	return 0
...
get_watson_message_()
{
	if [ "$RUN_RESULT" -gt 124 ] ; then
		...
		WATSON_MESSAGE_TEXT="FAILED: compressing $Input"
		return
	fi

	if [ "$RUN_RESULT" -ne 0 -a "$RETRY_NUM" -gt 0 -a $(( $RETRY_NUM % 10)) -eq 0 ] ; then
		WATSON_MESSAGE_ID="compress.$Input.$RETRY_NUM"
		WATSON_MESSAGE_TEXT="ERROR: compressing $Input [Retry #$RETRY_NUM]"
	fi
}
